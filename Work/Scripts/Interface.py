...
class Graphics(tk.Tk):
    ...
    def __init__(self, country: str, typef: str, disease_base: Database, par: Parser, saver: Saver):
        ...
        if flag:
            ...
            if typef == "disease":
            ...
            elif typef == "morediseases" or typef == "moredies":
                dates.pop(0)
                dates.pop()
                if typef == "moredies":
                ...
                stats1 = [int(item[1]) for item in info]
                ...
                predis = [int(item[1]) for item in info]
                moredis = []
...
class MainInterface(Frame):
    ...
    def init_ui(self):
        ...
        btn4.bind('<Button-1>', lambda a: Graphics.show(self.country, "morediseases", self.disease_base, self.par,
                                                        self.saver))
        btnscatter = Button(frame2, text="Заражения к смертям", width=30)
        btnscatter.bind('<Button-1>', lambda a: Graphics.show(self.country, "scatter", self.disease_base, self.par,
                                                              self.saver))
        btnbox = Button(frame2, text="Сравнение приростов", width=30)
        btnbox.bind('<Button-1>', lambda a: Graphics.show(self.country, "box", self.disease_base, self.par, self.saver))
        ...
        btnscatter.pack(padx=15, pady=15)
        btnbox.pack(padx=15, pady=15)
